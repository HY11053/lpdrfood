...
class SiteMapController extends Controller
{
    ...
    function Index()
    {
        ...
        for ($i=0;$i<count($newsurllinks);$i++)
        {
          $urlsets.="
<url>
    <loc>$appurl/{$newsurllinks[$i]->arctype->real_path}/{$newsurllinks[$i]->id}.shtml</loc>
    ...
    <priority>1.0</priority>
        ...
        for ($i=0;$i<count($brandlinks);$i++)
        {
            $urlsets.="
<url>
    <loc>$appurl/{$brandlinks[$i]->arctype->real_path}/{$brandlinks[$i]->id}.shtml</loc>
...
";
    ...
    function MobileSitemap()
    {
        ...
        foreach ($brandlinks as $brandlink){
            $sitemapinfos.="<url> 
            <loc>$mappurl/{$brandlink->arctype->real_path}/{$brandlink->id}.shtml</loc> 
            <mobile:mobile type=\"mobile\"/>
            <changefreq>daily</changefreq>
            <priority>1.0</priority>
            </url> 
            <url>
                <loc>$appurl/{$brandlink->arctype->real_path}/{$brandlink->id}.shtml</loc>
                <changefreq>daily</changefreq>                
                <priority>1.0</priority>
            </url>          
            ";
